...
#include "openarm_pid_controller/openarm_pid_controller.hpp"
...
using ControllerCommandMsg = openarm_pid_controller::PidController::ControllerReferenceMsg;
...
namespace openarm_pid_controller
{
...
{
  // Marker log to confirm this custom controller is the one being loaded,
  // not the stock pid_controller from /opt/ros.
  RCLCPP_INFO(get_node()->get_logger(), "OPENARM_HARDWARE_CONTROLLER PID");

  feedforward_mode_enabled_.initRT(false);

  try
  {
    param_listener_ = std::make_shared<openarm_pid_controller::ParamListener>(get_node());
  }
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // Every 10 s, log the active PID gains for all 7 joints on ONE line. params_
  // is refreshed by update_parameters() above, so this reflects live
  // `ros2 param set` changes, not just the values loaded at startup.
  //
  // Hardcoded to a single RCLCPP_INFO_THROTTLE call on purpose: the throttle
  // keeps its state per call-site, so a throttle inside a for-loop only ever
  // emits one joint per period. One call = the whole line clears the throttle
  // together.
  constexpr int GAIN_LOG_THROTTLE_MS = 10000;
  if (params_.dof_names.size() == 7)
  {
    const auto & g0 = params_.gains.dof_names_map[params_.dof_names[0]];
    const auto & g1 = params_.gains.dof_names_map[params_.dof_names[1]];
    const auto & g2 = params_.gains.dof_names_map[params_.dof_names[2]];
    const auto & g3 = params_.gains.dof_names_map[params_.dof_names[3]];
    const auto & g4 = params_.gains.dof_names_map[params_.dof_names[4]];
    const auto & g5 = params_.gains.dof_names_map[params_.dof_names[5]];
    const auto & g6 = params_.gains.dof_names_map[params_.dof_names[6]];
    RCLCPP_INFO_THROTTLE(
      get_node()->get_logger(), *get_node()->get_clock(), GAIN_LOG_THROTTLE_MS,
      "j1[p=%.3f i=%.3f d=%.3f] j2[p=%.3f i=%.3f d=%.3f] j3[p=%.3f i=%.3f d=%.3f] "
      "j4[p=%.3f i=%.3f d=%.3f] j5[p=%.3f i=%.3f d=%.3f] j6[p=%.3f i=%.3f d=%.3f] "
      "j7[p=%.3f i=%.3f d=%.3f]",
      g0.p, g0.i, g0.d, g1.p, g1.i, g1.d, g2.p, g2.i, g2.d,
      g3.p, g3.i, g3.d, g4.p, g4.i, g4.d, g5.p, g5.i, g5.d,
      g6.p, g6.i, g6.d);
  }
...
PLUGINLIB_EXPORT_CLASS(
  openarm_pid_controller::PidController, controller_interface::ChainableControllerInterface)
