...
class MultiPickController(Node):
    ...
    def graceful_shutdown(self):
        ...
        # FIX: Tăng thời gian chờ lên 15 giây.
        # Nếu đang lùi dở bằng CSV (is_recovery=True), nó sẽ đợi cho robot lùi hẳn về RAISE mới tắt tiến trình.
        wait_time = 0.0
        ...
        # 1. Nếu đang ở giữa đường gắp (Pre-Grasp, Grasp, Lift)
        ...
        # 2. Nếu vừa bị ngắt trong lúc đang lùi bằng CSV
    ...
    def run_sequence(self):
        ...
        while rclpy.ok() and not self.is_shutting_down:
            ...
            # Làm sạch history chuẩn bị plan cho vật mới
            self.trajectory_history.clear()
            ...
            # TRƯỚC KHI LÙI CSV: XÓA HISTORY MOVEIT ĐỂ TRÁNH LỖI KHI NHẤN CTRL+C DỞ DANG
            self.trajectory_history.clear()
            self.robot_state = "REVERSING_CSV"  # Báo cho hệ thống biết đang lùi bằng CSV

            # ========================================================
            # REVERSE CSV VỀ RAISE (BỎ QUA SEGMENT 0 VÀ 1)
            # ========================================================
            ...
            for seg in sorted_segments:
                ...
                # FIX Ở ĐÂY: Thêm is_recovery=True
                # Cờ này giúp robot KHÔNG bị ngắt ngang khi nhấn Ctrl+C, mà tiếp tục chạy nốt về RAISE
        ...
        # ========================================================
        # 4. TRỞ VỀ HOME (Chỉ áp dụng khi kết thúc tự nhiên, không do Ctrl+C)
        # ========================================================
