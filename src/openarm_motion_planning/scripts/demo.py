...
class MoveDualArmHybrid(Node):
    ...
    # ============================================================
    # PICK & PLACE COORDINATES UPDATED
    # ============================================================
    ...
    def manage_attached_object_sync(self, attach=True):
        ...
        if attach:
            ...
            # Kích thước vật mới: [0.15, 0.15, 0.1]
            box = SolidPrimitive(type=SolidPrimitive.BOX, dimensions=[0.15, 0.15, 0.1])
            box_pose = Pose()

            # Tay trái nằm ở Y = 0.075, tâm vật ở Y = 0.0 -> Khung local vật lùi đi một đoạn Y = -0.075 so với tay
    ...
    def planning_thread(self):
        ...
        for i in range(len(self.left_targets)):
            ...
            # 5. Logic Tag Hành Động
            if tag == "HOLD" and not is_holding:
                self.control_hands(close=True)
                time.sleep(1.0)  # Đợi tay đóng chắc
                self.manage_attached_object_sync(attach=True)
                is_holding = True

            elif tag == "WAIT_ENTER":
                # In thông báo ra màn hình màu vàng để dễ nhìn thấy
                print("\n\033[93m" + "=" * 50)
                print(" ĐÃ NHẤC VẬT. NHẤN PHÍM [ENTER] ĐỂ TIẾP TỤC PLACE! ")
                print("=" * 50 + "\033[0m\n")

                input()  # Lệnh này sẽ tạm ngưng luồng lập kế hoạch cho tới khi Enter được bấm
                ...
                time.sleep(1.0)  # Đợi tay mở bung ra
                is_holding = False
