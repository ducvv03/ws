...
int run_motor_state_control(const std::string& interface, bool use_arm_ids,
    ...
    try {
        ...
        for (size_t i = 0; i < motors.size(); ++i) {
            // A motor that never answered still holds its initial 0.0 state,
            // which is a perfectly finite number: only the frame counter tells
            // silence apart from a motor genuinely parked at the origin.
            if (!motors[i].has_received_state()) {
                no_response.push_back(send_ids[i]);
            } else if (motors[i].has_fault()) {
                faulted.push_back(i);
            }
        }
        ...
        if (!faulted.empty()) {
            std::cerr << "✗ Motor fault reported:\n";
            for (size_t i : faulted) {
                std::cerr << "  " << format_hex_id(send_ids[i]) << " : "
                          << openarm::damiao_motor::motor_status_to_string(motors[i].get_status())
                          << "\n";
            }
            std::cerr << "  Fix the cause, then run 'clear-error' to release the fault.\n";
            return 1;
        }
