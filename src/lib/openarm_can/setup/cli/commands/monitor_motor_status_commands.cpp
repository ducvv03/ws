...
int run_monitor(const std::string& interface, bool use_arm_ids,
    ...
    // A motor that stops answering keeps its last values forever, so treat
    // anything older than a few refresh cycles as no longer valid.
    constexpr int STALE_INTERVAL_COUNT = 3;
    const double stale_timeout_s = STALE_INTERVAL_COUNT * interval_ms / 1000.0;

    // Blank out a reading whose motor is silent, so fmtval renders it as "---"
    auto fresh_or_nan = [](bool fresh, double value) {
        return fresh ? value : std::numeric_limits<double>::quiet_NaN();
    };
    ...
    try {
        ...
        while (g_monitor_running) {
            ...
            std::cout << std::left << std::setw(8) << "ID" << std::setw(14) << "Pos(rad)"
                      ...
                      << std::setw(10) << "MOS(C)" << std::setw(8) << "Rtr(C)" << std::setw(16)
                      << "Status" << "Last fault\n";
            std::cout << "-----------------------------------------------------------"
                         "-----------\n";
            ...
            for (size_t i = 0; i < motors.size(); ++i) {
                ...
                const bool fresh = m.has_received_state() && !m.is_state_stale(stale_timeout_s);
                const auto last_fault = m.get_last_fault();
                ...
                std::cout << std::left << std::setfill(' ') << std::setw(8) << id_ss.str()
                          << fmtval(fresh_or_nan(fresh, m.get_position()), 14, 3)
                          << fmtval(fresh_or_nan(fresh, m.get_velocity()), 14, 3)
                          << fmtval(fresh_or_nan(fresh, m.get_torque()), 14, 3)
                          << fmtval(fresh_or_nan(fresh, m.get_state_tmos()), 10, 1)
                          << fmtval(fresh_or_nan(fresh, m.get_state_trotor()), 8, 1)
                          << std::setw(16)
                          << (fresh ? openarm::damiao_motor::motor_status_to_string(m.get_status())
                                    : "NO RESPONSE")
                          << (last_fault ? openarm::damiao_motor::motor_status_to_string(*last_fault)
                                         : "-")
                          << "\n";
            }
            std::cout << "==========================================================="
