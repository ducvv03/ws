...
StateResult CanPacketDecoder::parse_motor_state_data(const Motor& motor,
    ...
    if (data.size() < FEEDBACK_FRAME_MIN_BYTES) {
        std::cerr << "Warning: Skipping motor state data less than " << FEEDBACK_FRAME_MIN_BYTES
                  << " bytes (got " << data.size() << ")" << std::endl;
        return {0, 0, 0, 0, 0, false, MotorStatus::UNKNOWN, 0};
    }

    // D[0] carries the motor status nibble. The frame is already routed by CAN
    // ID in DMCANDevice::callback, so the ID nibble is redundant for dispatch;
    // it is handed to the caller inside raw_status_byte instead of gating the
    // frame here, which keeps an unexpected encoding from dropping all data.
    uint8_t raw_status_byte = data[0];
    MotorStatus status = to_motor_status(
        static_cast<uint8_t>((raw_status_byte >> FEEDBACK_STATUS_NIBBLE_SHIFT) &
                             FEEDBACK_NIBBLE_MASK));
    ...
    return {recv_q, recv_dq, recv_tau, t_mos, t_rotor, true, status, raw_status_byte};
}

ParamResult CanPacketDecoder::parse_motor_param_data(const std::vector<uint8_t>& data) {
    if (data.size() < FEEDBACK_FRAME_MIN_BYTES) {
        std::cerr << "Warning: Skipping motor param data less than " << FEEDBACK_FRAME_MIN_BYTES
                  << " bytes (got " << data.size() << ")" << std::endl;
        return {0, NAN, false};
    }
