...
1. Opens a `dora.Ros2Context()` and always creates the `/vr_buttons` (`sensor_msgs/Joy`) publisher and the `/right_ee_pose`/`/left_ee_pose` (`geometry_msgs/PoseStamped`) publishers, plus mode-specific publisher(s) — see the input/output table in `README.md` for the full mapping. The ee_pose publishers only ever emit anything if the yaml wires `ee_pose_right`/`ee_pose_left` inputs from a `dora-openarm-fk` node (all three current yamls do); that node computes FK from `ik`'s `position_right`/`position_left` output using the same vendored `dora-openarm-kinematics` package (no MuJoCo/mink dependency needed in this package itself). Both use best-effort QoS for the same reason as buttons (`qos_buttons`): no guarantee anything's subscribed, and reliable QoS + no subscriber previously caused a real publish-timeout crash (see the button-QoS note below).
