...
//
// Standalone C++11 port: see include/looper/Looper.h for the list of
// libutils dependencies that were removed.
//
...
// --- Minimal logging, replacing libutils' ALOG*/LOG_ALWAYS_FATAL* ---

#define LOOPER_LOG(level, ...)                  \
    do {                                        \
        std::fprintf(stderr, "Looper " level ": "); \
        std::fprintf(stderr, __VA_ARGS__);      \
        std::fprintf(stderr, "\n");             \
    } while (0)

#define ALOGW(...) LOOPER_LOG("W", __VA_ARGS__)
#define ALOGE(...) LOOPER_LOG("E", __VA_ARGS__)
#define ALOGD(...) LOOPER_LOG("D", __VA_ARGS__)

#define LOG_ALWAYS_FATAL(...)              \
    do {                                   \
        LOOPER_LOG("FATAL", __VA_ARGS__);  \
        std::abort();                      \
    } while (0)
...
epoll_event createEpollEvent(uint32_t events, uint64_t seq) {
    ...
    eventItem.data.u64 = seq;
    return eventItem;
}

// Replaces libutils systemTime(SYSTEM_TIME_MONOTONIC).
nsecs_t systemTimeMonotonic() {
    return std::chrono::duration_cast<std::chrono::nanoseconds>(
                   std::chrono::steady_clock::now().time_since_epoch())
            .count();
}

// Replaces libutils milliseconds_to_nanoseconds().
constexpr nsecs_t milliseconds_to_nanoseconds(nsecs_t ms) {
    return ms * 1000000LL;
}

// Replaces libutils toMillisecondTimeoutDelay() (rounds up, clamps to INT_MAX).
...
void unique_fd::reset(int newFd) {
    if (mFd >= 0) {
        ::close(mFd);
    }
    mFd = newFd;
...
WeakMessageHandler::WeakMessageHandler(const std::weak_ptr<MessageHandler>& handler)
    : mHandler(handler) {}

WeakMessageHandler::~WeakMessageHandler() {}

void WeakMessageHandler::handleMessage(const Message& message) {
    std::shared_ptr<MessageHandler> handler = mHandler.lock();
    if (handler != nullptr) {
...
thread_local static std::shared_ptr<Looper> gThreadLocalLooper;
...
Looper::Looper(bool allowNonCallbacks)
    ...
    LOG_ALWAYS_FATAL_IF(mWakeEventFd.get() < 0, "Could not make wake event fd: %s",
                        strerror(errno));
...
std::shared_ptr<Looper> Looper::prepare(int opts) {
    bool allowNonCallbacks = opts & PREPARE_ALLOW_NON_CALLBACKS;
    std::shared_ptr<Looper> looper = Looper::getForThread();
    if (looper == nullptr) {
        looper = std::make_shared<Looper>(allowNonCallbacks);
...
void Looper::rebuildEpollLocked() {
    ...
    LOG_ALWAYS_FATAL_IF(mEpollFd.get() < 0, "Could not create epoll instance: %s",
                        strerror(errno));
    ...
    for (const auto& entry : mRequests) {
        const SequenceNumber seq = entry.first;
...
int Looper::pollInner(int timeoutMillis) {
    ...
    if (timeoutMillis != 0 && mNextMessageUptime != LLONG_MAX) {
        nsecs_t now = systemTimeMonotonic();
    ...
    for (int i = 0; i < eventCount; i++) {
        ...
        if (seq == WAKE_EVENT_FD_SEQ) {
            ...
            if (request_it != mRequests.end()) {
                ...
                mResponses.push_back(Response{seq, events, request});
            } else {
...
Done:;
    ...
    while (mMessageEnvelopes.size() != 0) {
        nsecs_t now = systemTimeMonotonic();
        const MessageEnvelope& messageEnvelope = mMessageEnvelopes[0];
    ...
    for (size_t i = 0; i < mResponses.size(); i++) {
        ...
        if (response.request.ident == POLL_CALLBACK) {
            ...
            response.request.callback.reset();
            result = POLL_CALLBACK;
        }
    }
    return result;
...
int Looper::pollAll(int timeoutMillis, int* outFd, int* outEvents, void** outData) {
    if (timeoutMillis <= 0) {
        ...
        nsecs_t endTime =
                systemTimeMonotonic() + milliseconds_to_nanoseconds(timeoutMillis);
...
int Looper::addFd(int fd, int ident, int events, Looper_callbackFunc callback, void* data) {
    std::shared_ptr<SimpleLooperCallback> looperCallback;
    if (callback) {
        looperCallback = std::make_shared<SimpleLooperCallback>(callback);
    }
    return addFd(fd, ident, events, looperCallback, data);
}

int Looper::addFd(int fd, int ident, int events, const std::shared_ptr<LooperCallback>& callback,
                  void* data) {
#if DEBUG_CALLBACKS
...
bool Looper::getFdStateDebug(int fd, int* ident, int* events,
                             std::shared_ptr<LooperCallback>* cb, void** data) {
    std::lock_guard<std::mutex> _l(mLock);
    auto seqNumIt = mSequenceNumberByFd.find(fd);
    if (seqNumIt != mSequenceNumberByFd.cend()) {
        auto reqIt = mRequests.find(seqNumIt->second);
        if (reqIt != mRequests.cend()) {
...
int Looper::repoll(int fd) {
    ...
    const SequenceNumber seq = request_it->first;
...
void Looper::sendMessageDelayed(nsecs_t uptimeDelay, const std::shared_ptr<MessageHandler>& handler,
                                const Message& message) {
    nsecs_t now = systemTimeMonotonic();
    sendMessageAtTime(now + uptimeDelay, handler, message);
}

void Looper::sendMessageAtTime(nsecs_t uptime, const std::shared_ptr<MessageHandler>& handler,
                               const Message& message) {
#if DEBUG_CALLBACKS
...
void Looper::removeMessages(const std::shared_ptr<MessageHandler>& handler) {
    ...
    {  // acquire lock
        ...
        for (size_t i = mMessageEnvelopes.size(); i != 0;) {
            const MessageEnvelope& messageEnvelope = mMessageEnvelopes[--i];
